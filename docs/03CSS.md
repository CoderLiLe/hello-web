...
- [10 浮动](#10-浮动)
  - [10.1 浮动的简介](#101-浮动的简介)
  - [10.2 元素浮动之后的特点](#102-元素浮动之后的特点)
  - [10.3 浮动元素产生的影响](#103-浮动元素产生的影响)
    - [① 对后面兄弟元素的影响](#-对后面兄弟元素的影响)
    - [② 对父元素的影响](#-对父元素的影响)
  - [10.4 浮动相关的 CSS 属性](#104-浮动相关的-css-属性)
- [11 行内元素或行内块元素在布局中的特点](#11-行内元素或行内块元素在布局中的特点)
  - [11.1 父元素设置的文本属性可以作用于行内元素和行内块元素](#111-父元素设置的文本属性可以作用于行内元素和行内块元素)
    - [① 让行内块元素在父元素中水平居中](#-让行内块元素在父元素中水平居中)
    - [② 让行内块元素在父元素中纵向居中](#-让行内块元素在父元素中纵向居中)
  - [11.2 行内元素或行内块元素之间的空白问题](#112-行内元素或行内块元素之间的空白问题)
    - [① 元素之间的空白（左右）](#-元素之间的空白左右)
    - [② 底部的空白（图片的幽灵空白）](#-底部的空白图片的幽灵空白)
    - [③ 文字内容个数不同的行内块元素水平排列无法对齐](#-文字内容个数不同的行内块元素水平排列无法对齐)
- [12 定位](#12-定位)
  - [12.1 相对定位](#121-相对定位)
    - [① 如何设置相对定位](#-如何设置相对定位)
    - [② 相对定位元素定位的参考点](#-相对定位元素定位的参考点)
    - [③ 相对定位元素的特点](#-相对定位元素的特点)
  - [12.2 绝对定位](#122-绝对定位)
    - [① 如何设置绝对定位](#-如何设置绝对定位)
    - [② 绝对定位元素定位的参考点](#-绝对定位元素定位的参考点)
    - [③ 绝对定位元素的特点](#-绝对定位元素的特点)
  - [12.3 固定定位](#123-固定定位)
    - [① 如何设置为固定定位](#-如何设置为固定定位)
    - [② 固定定位的元素定位参考点](#-固定定位的元素定位参考点)
    - [③ 固定定位元素的特点（同绝对定位）](#-固定定位元素的特点同绝对定位)
  - [12.4 定位显示层级 z-index](#124-定位显示层级-z-index)
  - [12.5 定位相关 CSS 属性](#125-定位相关-css-属性)
  - [12.6 定位元素（绝对和固定）的默认宽高计算规则](#126-定位元素绝对和固定的默认宽高计算规则)
  - [12.7 设置定位元素（绝对和固定）在包含块中水平垂直都居中](#127-设置定位元素绝对和固定在包含块中水平垂直都居中)
- [13 精灵图](#13-精灵图)
  - [13.1 什么是精灵图](#131-什么是精灵图)
  - [13.2 精灵图的优点](#132-精灵图的优点)
  - [13.3 制作精灵图在线工具](#133-制作精灵图在线工具)
- [14 页面布局](#14-页面布局)
  - [14.1 页面的组成部分](#141-页面的组成部分)
  - [14.2 重置样式表](#142-重置样式表)
  - [14.3 版心 container](#143-版心-container)
  - [14.4 编码规范](#144-编码规范)
  - [14.5 网站标题图标](#145-网站标题图标)
    - [① favicon 图标的使用](#-favicon-图标的使用)
    - [② favicon 图标的制作](#-favicon-图标的制作)
...
### 11.1 父元素设置的文本属性可以作用于行内元素和行内块元素
...
### 12.7 设置定位元素（绝对和固定）在包含块中水平垂直都居中
