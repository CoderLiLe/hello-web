...
- [配置别名路径](#配置别名路径)
  ...
  - [3. 联想路径配置](#3-联想路径配置)
- [数据Mock实现](#数据mock实现)
  - [1. 常见的Mock方式](#1-常见的mock方式)
  - [2. json-server实现Mock](#2-json-server实现mock)
- [整体路由设计](#整体路由设计)
- [antD主题定制](#antd主题定制)
  - [1. 定制方案](#1-定制方案)
  - [2. 实现方式](#2-实现方式)
  - [3. 记账本主题色](#3-记账本主题色)
- [Redux管理账目列表](#redux管理账目列表)
- [TabBar功能实现](#tabbar功能实现)
  - [1. 静态布局实现](#1-静态布局实现)
  - [2. 切换路由实现](#2-切换路由实现)
- [月度账单-统计区域](#月度账单-统计区域)
  - [1. 准备静态结构](#1-准备静态结构)
  - [2. 点击切换时间选择框](#2-点击切换时间选择框)
  - [3. 切换时间显示](#3-切换时间显示)
  - [4. 统计功能实现](#4-统计功能实现)
  - [5. 完整代码](#5-完整代码)
- [月度账单-单日统计列表实现](#月度账单-单日统计列表实现)
  - [1. 准备组件和配套样式](#1-准备组件和配套样式)
  - [2. 按日分组账单数据](#2-按日分组账单数据)
  - [3. 遍历日账单组件并传入参数](#3-遍历日账单组件并传入参数)
  - [4. 接收数据计算统计渲染页面](#4-接收数据计算统计渲染页面)
- [月度账单-单日账单列表展示](#月度账单-单日账单列表展示)
  - [1. 渲染基础列表](#1-渲染基础列表)
  - [2. 适配Type](#2-适配type)
- [月度账单-切换打开关闭](#月度账单-切换打开关闭)
- [月度账单-Icon组件封装](#月度账单-icon组件封装)
  - [1. 准备静态结构](#1-准备静态结构-1)
  - [2. 设计参数](#2-设计参数)
  - [3. 使用组件](#3-使用组件)
- [记账功能](#记账功能)
  - [记账 - 结构渲染](#记账---结构渲染)
  - [记账 - 支出和收入切换](#记账---支出和收入切换)
  - [记账 - 新增一笔](#记账---新增一笔)
...
![image.png](assets/04/15.png)

4.  访问接口进行测试 
# 整体路由设计
![image.png](assets/04/16.png)
1. 俩个一级路由 （Layout / new）2. 俩个二级路由 （Layout - mouth/year）

# antD主题定制
## 1. 定制方案
![image.png](assets/04/17.png)
## 2. 实现方式

1. 全局定制

![image.png](assets/04/18.png)

2. 局部定制

![image.png](assets/04/19.png)
